...
import time
import pandas as pd
from output_file_names_packing_binary_small_scale import output_name_Sec32_Shift
global df
...
def knap(c,A,b,m,n,p,N,t0):
    ...
    tt = M.addMVar(p,  lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS)
    ...
    if N>=1:
        ...
        M.addConstrs(tt[i]==(c[i,:] @ x-t0[i]) for i in range(p))
        M.addConstrs(r[i,N-1]*2**(2*N+1)>=(2**(2*N+1)+2**(N+1)*tt[i]+tt[i]@tt[i]) for i in range(p))
        M.addConstrs((t[i]>=r[i,0]@r[i,0] for i in range(p)))
        M.addConstrs((r[i,k]>= r[i,k+1]@r[i,k+1] for k in range(N-1) for i in range(p)))
    else:
        M.addConstrs(tt[i]==(c[i,:] @ x-t0[i]) for i in range(p))
        M.addConstrs((t[i]*2>=(2+2*tt[i]+tt[i]@tt[i]) for i in range(p)))
    return M, x
...
for p in range(5, 27, 2):
    ...
    for N in [1, 3, 5]:
        ...
        df.to_csv(output_name_Sec32_Shift+'.csv')
