...
for p in range(5, 55, 5):
    ...
    b=np.array([2*n]*m)
    for sa,kk in [(x,y) for x in [-1] for y in [1]]:
        sa0 = sa
        ...
        df.loc[instance_number] =np.array([m,n,p,kk,a,sa0,UB,LB,Mtime_total,Gap,EUB,Approx_Gap])
        df.to_csv('gurobi_covering_exmp3_best_scale_binary.csv')
