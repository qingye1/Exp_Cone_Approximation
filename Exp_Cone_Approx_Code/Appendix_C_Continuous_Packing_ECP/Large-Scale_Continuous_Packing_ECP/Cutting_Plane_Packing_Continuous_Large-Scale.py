...
import time
import pandas as pd
from output_file_names_packing_continuous_large_scale import output_name_CP
global df
...
def knap(c,A,b,m,n,p):
    ...
    M.params.OutputFlag = 0
    check=0
    while (iteration<=maxiter) and (UB-LB)/abs(UB)>1e-5 and time.time() - start<=3600:
