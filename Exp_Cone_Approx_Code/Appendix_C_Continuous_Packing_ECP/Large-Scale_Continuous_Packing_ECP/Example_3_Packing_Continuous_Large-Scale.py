...
import time
import pandas as pd
from output_file_names_packing_continuous_large_scale import output_name_Ex3
global df
...
a=1.0
instance_number=0
for m in range(200, 2200, 200):
    n = int(m)
    for p in range(200, 2200, 200):
        c=-np.random.randint(10, size=(p, n))/float(n)
        A=np.random.randint(10, size=(m, n))
        b=np.array([4*n]*m)
        for sa,kk in [(x,y) for x in [-4] for y in [5]]:
            ...
            df.loc[instance_number] =np.array([m,n,p,kk,a,sa, UB, Mtime, EUB, Approx_Gap])
            df.to_csv(output_name_Ex3+'.csv')
